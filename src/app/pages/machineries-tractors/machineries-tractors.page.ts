...
export class MachineriesTractorsPage implements OnInit {
    ...
    addSpecification() {
      if (this.tractors.specification) {
        this.specifications.push(this.tractors.specification);
        this.tractors.specification = ''; // Clear the input after adding
      }
    }
        ...
        this.http.post('http://localhost/Govi-Nena-Home-Garden-Advertisement-Module-Backend/add_tractors.php', formData)
          .subscribe({
            next: (response) => {
              ...
              alert('Tractors advertisement successfully submitted.');
            },
            error: (error) => {
