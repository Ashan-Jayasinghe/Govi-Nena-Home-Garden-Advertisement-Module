...
export class MachineriesSprayersPage implements OnInit {
    ...
    addSpecification() {
      if (this.sprayers.specification) {
        this.specifications.push(this.sprayers.specification);
        this.sprayers.specification = ''; // Clear the input after adding
      }
    }
        ...
        this.http.post('http://localhost/Govi-Nena-Home-Garden-Advertisement-Module-Backend/add_sprayers.php', formData)
          .subscribe({
            next: (response) => {
              ...
              alert('Sprayers advertisement successfully submitted.');
            },
            error: (error) => {
